...
void Particle::setModeBeforePause(particleMode mode){
	lastMode = mode;
}
...
//------------------------------------------------------------------
void Particle::draw(){
	if( mode == PARTICLE_MODE_ATTRACT || lastMode == PARTICLE_MODE_ATTRACT ){
		ofSetColor(255, 63, 180);
	}
	else if( mode == PARTICLE_MODE_REPEL || lastMode == PARTICLE_MODE_REPEL ){
		ofSetColor(208, 255, 63);
	}
	else if( mode == PARTICLE_MODE_NOISE || lastMode == PARTICLE_MODE_NOISE ){
		ofSetColor(99, 63, 255);
	}
	else if( mode == PARTICLE_MODE_NEAREST_POINTS || lastMode == PARTICLE_MODE_NEAREST_POINTS ){
		ofSetColor(103, 160, 237);
	}
	ofDrawCircle(pos.x, pos.y, scale * 4.0);
}

void Particle::changeParticleColor(int countTpresses){

	switch(countTpresses){
		case 1:
			ofSetColor(255, 0, 0);
			break;
		case 2:
...
void Particle::magnifyParticle(bool dragRectangle, int originalMouseX, int newMouseX, int originalMouseY, int newMouseY){
	if(dragRectangle && (min(originalMouseX, newMouseX) <= pos.x) && (pos.x <= max(originalMouseX, newMouseX)) && (min(originalMouseY, newMouseY) <= pos.y) && (pos.y <= max(originalMouseY, newMouseY))){
		ofDrawCircle(pos.x, pos.y, scale * 12.0);
	}
}